...
function BuilderView({ project, theme, selectedPage, selectedSection, onPage, onSection, onAddPage, onAddSection, onProjectUpdate, onSectionAction, onPreview }: { project: Project; theme: Theme; selectedPage: Page; selectedSection: string; onPage: (id: string) => void; onSection: (section: string) => void; onAddPage: () => void; onAddSection: (sectionName?: string) => void; onProjectUpdate: (field: 'title' | 'date' | 'location' | 'tagline' | 'description' | 'ctaLabel', value: string) => void; onSectionAction: (sectionName: string, action: 'duplicate' | 'delete' | 'up' | 'down') => void; onPreview: () => void }) {
  ...
  return <div className="framer-editor-body">
    <aside className="framer-left-panel">
      <div className="framer-panel-tabs"><button type="button" className={leftMode === 'pages' ? 'active' : ''} onClick={() => setLeftMode('pages')}><Icon name="layers" size={15} />Pages</button><button type="button" className={leftMode === 'layers' ? 'active' : ''} onClick={() => setLeftMode('layers')}><Icon name="grip" size={15} />Layers</button><button type="button" className={leftMode === 'assets' ? 'active' : ''} onClick={() => setLeftMode('assets')}><Icon name="image" size={15} />Assets</button></div>
      <label className="framer-search"><Icon name="search" size={15} /><input placeholder="Search..." /></label>
      {leftMode === 'pages' && <><div className="framer-panel-section-title"><span>Design</span><IconButton name="plus" label="Ajouter un élément" size={15} /></div><div className="framer-tool-row"><button type="button"><Icon name="type" size={15} /><span>Text</span></button><button type="button"><Icon name="image" size={15} /><span>Media</span></button><button type="button" onClick={() => setShowSectionLibrary(true)}><Icon name="layout" size={15} /><span>Section</span></button></div><div className="framer-panel-section-title pages-title"><span>Pages</span><IconButton name="plus" label="Ajouter une page" size={15} onClick={onAddPage} /></div><div className="framer-page-list">{project.pages.map((page) => <button type="button" key={page.id} className={`framer-page-item ${selectedPage.id === page.id ? 'active' : ''}`} onClick={() => onPage(page.id)}><span className="framer-page-icon"><Icon name={page.icon} size={14} /></span><span>{page.name}</span>{page.id === 'home' && <span className="framer-home-glyph">⌂</span>}</button>)}</div><button type="button" className="framer-add-page" onClick={onAddPage}><Icon name="plus" size={14} />Add page</button></>}
      {leftMode === 'layers' && <><div className="framer-panel-section-title"><span>{selectedPage.name} layers</span><IconButton name="plus" label="Ajouter une section" size={15} onClick={() => setShowSectionLibrary(true)} /></div><div className="framer-layer-list">{selectedPage.sections.map((section, index) => <div key={`${section}-${index}`} className={`framer-layer-item ${selectedSection === section ? 'active' : ''}`}><button type="button" className="layer-select" onClick={() => { onSection(section); setPanelMode('style') }}><Icon name="grip" size={13} /><span>{section}</span></button><div className="layer-actions"><button type="button" onClick={() => onSectionAction(section, 'up')} aria-label="Monter"><Icon name="chevron-left" size={11} /></button><button type="button" onClick={() => onSectionAction(section, 'down')} aria-label="Descendre"><Icon name="chevron-right" size={11} /></button><button type="button" onClick={() => onSectionAction(section, 'duplicate')} aria-label="Dupliquer"><Icon name="copy" size={11} /></button><button type="button" onClick={() => onSectionAction(section, 'delete')} aria-label="Supprimer"><Icon name="trash" size={11} /></button></div></div>)}</div><button type="button" className="framer-add-page" onClick={() => setShowSectionLibrary(true)}><Icon name="plus" size={14} />Add section</button></>}
      {leftMode === 'assets' && <><div className="framer-panel-section-title"><span>Project assets</span><IconButton name="plus" label="Ajouter un média" size={15} /></div><div className="framer-assets-grid"><button type="button" className="asset-placeholder"><Icon name="plus" size={16} /><span>Upload</span></button><div className="asset-tile asset-tile-one"><span>IMG / 01</span></div><div className="asset-tile asset-tile-two"><span>IMG / 02</span></div><div className="asset-tile asset-tile-three"><span>IMG / 03</span></div></div></>}
