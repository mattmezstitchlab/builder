...
function ProjectNavigator({ projects, activeProjectId, onOpenProject, onNewProject, search, onSearch }: { projects: Project[]; activeProjectId: string | null; onOpenProject: (id: string, tab?: EditorTab) => void; onNewProject: () => void; search: string; onSearch: (value: string) => void }) {
  const [folder, setFolder] = useState<'all' | 'wedding' | 'other'>('all')
  const visibleProjects = projects.filter((project) => {
    const matchesFolder = folder === 'all' || (folder === 'wedding' ? project.eventType === 'wedding' : project.eventType !== 'wedding')
    return matchesFolder && `${project.title} ${project.subtitle} ${project.location}`.toLowerCase().includes(search.toLowerCase())
  })
  return <aside className="project-navigator"><div className="project-navigator-head"><div><span className="eyebrow"><Icon name="grid" size={13} />Workspace</span><h2>Projets</h2></div><IconButton name="plus" label="Nouvel événement" size={16} onClick={onNewProject} /></div><label className="project-navigator-search"><Icon name="search" size={14} /><input value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Rechercher un projet" /></label><div className="project-tree"><button type="button" className={folder === 'all' ? 'active' : ''} onClick={() => setFolder('all')}><Icon name="chevron-down" size={12} /><Icon name="folder" size={14} /><span>Tous les projets</span><small>{projects.length}</small></button><button type="button" className={folder === 'wedding' ? 'active' : ''} onClick={() => setFolder('wedding')}><Icon name="chevron-right" size={12} /><Icon name="folder" size={14} /><span>Mariages 2026</span><small>{projects.filter((project) => project.eventType === 'wedding').length}</small></button><button type="button" className={folder === 'other' ? 'active' : ''} onClick={() => setFolder('other')}><Icon name="chevron-right" size={12} /><Icon name="folder" size={14} /><span>Autres événements</span><small>{projects.filter((project) => project.eventType !== 'wedding').length}</small></button><button type="button" onClick={() => notify('Les inspirations seront accessibles dans un dossier dédié.')}><Icon name="chevron-right" size={12} /><Icon name="folder" size={14} /><span>Inspirations</span></button></div><div className="project-navigator-label"><span>Projets récents</span><button type="button" onClick={onNewProject}><Icon name="plus" size={13} /></button></div><div className="project-tree-list">{visibleProjects.map((project) => <button type="button" key={project.id} className={`project-tree-card ${activeProjectId === project.id ? 'active' : ''}`} onClick={() => onOpenProject(project.id, 'build')}><div className={`project-tree-thumb ${project.thumbClass}`}><span>{project.initials}</span></div><span className="project-tree-card-copy"><strong>{project.title}</strong><small>{project.subtitle} · {project.status}</small></span><Icon name="chevron-right" size={13} /></button>)}{visibleProjects.length === 0 && <div className="project-tree-empty">Aucun projet dans ce dossier.</div>}</div><button type="button" className="project-navigator-new" onClick={onNewProject}><Icon name="plus" size={14} />Nouveau projet</button></aside>
}
...
function GlobalHeader({ project, onBack, onNew }: { project?: Project; onBack: () => void; onNew?: () => void }) {
  return <div className="framer-project-tabbar universal-header"><div className="framer-window-brand"><button type="button" className="window-dot dot-red" aria-label="Fermer" onClick={onBack} /><button type="button" className="window-dot dot-yellow" aria-label="Réduire" onClick={() => notify('Le mode réduction sera disponible dans l’application desktop.')} /><button type="button" className="window-dot dot-green" aria-label="Agrandir" onClick={() => notify('Le projet est déjà ouvert dans son espace maximum.')} /></div><div className="framer-app-tabs"><button type="button" className="framer-home-tab" onClick={onBack}><BrandMark /><span>atelier</span></button><button type="button" className="framer-file-tab muted-tab" onClick={onBack}><span className="file-tab-icon"><Icon name="edit" size={12} /></span><span>All projects</span></button>{project ? <button type="button" className="framer-file-tab active" onClick={() => notify(`Projet actif : ${project.title}`)}><span className="file-tab-icon"><Icon name="layers" size={12} /></span><span>{project.title}</span><Icon name="x" size={12} /></button> : <button type="button" className="framer-file-tab active workspace-tab" onClick={() => notify('Workspace Atelier actif.')}><span className="file-tab-icon"><Icon name="grid" size={12} /></span><span>Workspace</span></button>}<button type="button" className="framer-add-tab" aria-label="Nouvel onglet" onClick={onNew ?? (() => notify('Créez un nouvel événement depuis le dashboard.'))}><Icon name="plus" size={16} /></button></div><div className="framer-tabbar-spacer" /></div>
}
