...
function BuilderView({ project, theme, selectedPage, selectedSection, onPage, onSection, onAddPage, onAddSection, onPreview }: { project: Project; theme: Theme; selectedPage: Page; selectedSection: string; onPage: (id: string) => void; onSection: (section: string) => void; onAddPage: () => void; onAddSection: () => void; onPreview: () => void }) {
  const [panelMode, setPanelMode] = useState<'agent' | 'style'>('agent')

  return <div className="framer-editor-body">
    <aside className="framer-left-panel">
      <div className="framer-panel-tabs"><button type="button" className="active"><Icon name="layers" size={15} />Pages</button><button type="button"><Icon name="grip" size={15} />Layers</button><button type="button"><Icon name="image" size={15} />Assets</button></div>
      <label className="framer-search"><Icon name="search" size={15} /><input placeholder="Search..." /></label>
      <div className="framer-panel-section-title"><span>Design</span><IconButton name="plus" label="Ajouter un élément" size={15} /></div>
      <div className="framer-tool-row"><button type="button"><Icon name="type" size={15} /><span>Text</span></button><button type="button"><Icon name="image" size={15} /><span>Media</span></button><button type="button"><Icon name="layout" size={15} /><span>Section</span></button></div>
      <div className="framer-panel-section-title pages-title"><span>Pages</span><IconButton name="plus" label="Ajouter une page" size={15} onClick={onAddPage} /></div>
      <div className="framer-page-list">{project.pages.map((page) => <button type="button" key={page.id} className={`framer-page-item ${selectedPage.id === page.id ? 'active' : ''}`} onClick={() => onPage(page.id)}><span className="framer-page-icon"><Icon name={page.icon} size={14} /></span><span>{page.name}</span>{page.id === 'home' && <span className="framer-home-glyph">⌂</span>}</button>)}</div>
      <button type="button" className="framer-add-page" onClick={onAddPage}><Icon name="plus" size={14} />Add page</button>
      <div className="framer-left-footer"><button type="button"><Icon name="settings" size={15} />Settings</button><span>⌘ K</span></div>
    </aside>
    <main className="framer-canvas-area">
      <div className="framer-canvas-top"><div className="framer-canvas-size"><Icon name="play" size={13} /><span>Desktop</span><strong>1200</strong><Icon name="plus" size={15} /></div><div className="framer-canvas-breadcrumb"><span>{project.title}</span><Icon name="chevron-right" size={12} /><strong>{selectedPage.name}</strong></div></div>
      <div className="framer-stage"><div className="framer-canvas-ruler"><span>0</span><span>320</span><span>640</span><span>960</span><span>1200</span></div><div className="site-canvas framer-site-canvas" style={{ background: theme.background, color: theme.ink }}><div className="site-canvas-nav"><span className="site-logo-mark" style={{ color: theme.accent }}>✦</span><strong>{project.title}</strong><div><span>Programme</span><span>Infos</span><span>RSVP</span><button type="button" style={{ background: theme.accent, color: theme.background }}>Menu</button></div></div><div className="canvas-section selected-section" onClick={() => { onSection('Hero événement'); setPanelMode('style') }}><span className="section-tag">Hero événement <Icon name="edit" size={12} /></span><div className="canvas-hero-copy"><span className="canvas-eyebrow" style={{ color: theme.accent }}>{project.subtitle.toUpperCase()}</span><h1 style={{ fontFamily: theme.displayFont }}>{project.title.split(' & ')[0]} <em>&</em><br />{project.title.split(' & ')[1] ?? 'vous'}</h1><p>{project.date} <span>·</span> {project.location}</p><button type="button" style={{ background: theme.accent, color: theme.background }}>Découvrir l’événement <Icon name="arrow-right" size={14} /></button></div><div className="canvas-hero-art" style={{ background: theme.accentSoft }}><div className="hero-art-frame" style={{ borderColor: theme.accent }}><span>THE<br />MOMENT</span></div></div></div><div className="canvas-section canvas-info-section" onClick={() => { onSection('Date & compte à rebours'); setPanelMode('style') }}><span className="section-tag">Date & compte à rebours <Icon name="edit" size={12} /></span><div><span className="canvas-eyebrow" style={{ color: theme.accent }}>À RETENIR</span><h2 style={{ fontFamily: theme.displayFont }}>Un moment à partager.</h2></div><div className="date-boxes"><div><strong>12</strong><span>JOURS</span></div><div><strong>09</strong><span>MOIS</span></div><div><strong>26</strong><span>ANNÉE</span></div></div></div><button type="button" className="canvas-add-section" onClick={(event) => { event.stopPropagation(); onAddSection() }}><Icon name="plus" size={15} />Ajouter une section</button><div className="canvas-section canvas-story-section" onClick={() => { onSection('Notre histoire'); setPanelMode('style') }}><span className="section-tag">Notre histoire <Icon name="edit" size={12} /></span><div className="story-image" style={{ background: `linear-gradient(145deg, ${theme.accentSoft}, ${theme.accent}55)` }}><span>IMAGE / 01</span></div><div className="story-copy"><span className="canvas-eyebrow" style={{ color: theme.accent }}>LE CONTEXTE</span><h2 style={{ fontFamily: theme.displayFont }}>Tout commence<br />par une histoire.</h2><p>Ajoutez votre texte, vos images et le ton qui ressemble à votre événement.</p></div></div></div></div>
      <div className="framer-bottom-bar"><div className="framer-bottom-tools"><IconButton name="arrow-right" label="Sélection" active size={15} /><IconButton name="grip" label="Déplacer" size={15} /><span className="bottom-divider" /><IconButton name="sun" label="Mode clair" size={15} /></div><div className="framer-zoom"><button type="button">50% <Icon name="chevron-down" size={12} /></button><IconButton name="sliders" label="Options d'affichage" size={15} /></div><button type="button" className="framer-upgrade"><Icon name="zap" size={13} />Upgrade now</button></div>
    </main>
    <aside className="framer-right-panel">
      <div className="framer-right-tabs"><button type="button" className={panelMode === 'agent' ? 'active' : ''} onClick={() => setPanelMode('agent')}>Agent</button><button type="button" className={panelMode === 'style' ? 'active' : ''} onClick={() => setPanelMode('style')}>Style</button></div>
      {panelMode === 'agent' ? <AgentPanel project={project} onStyle={() => setPanelMode('style')} /> : <StyleInspector project={project} theme={theme} selectedSection={selectedSection} />}
    </aside>
  </div>
}
