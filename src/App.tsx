...
function App() {
  ...
  function addSection() {
    updateActiveProject((project) => ({
      ...project,
      pages: project.pages.map((page) => page.id === selectedPageId ? { ...page, sections: [...page.sections, 'Nouvelle section'] } : page),
      progress: Math.min(100, project.progress + 2),
    }))
    setSelectedSection('Nouvelle section')
  }
...
function Editor({ project, theme, editorTab, selectedPageId, selectedSection, onBack, onTab, onPage, onSection, onAddPage, onAddSection, onTheme, onPreview, onPublish }: { project: Project; theme: Theme; editorTab: EditorTab; selectedPageId: string; selectedSection: string; onBack: () => void; onTab: (tab: EditorTab) => void; onPage: (id: string) => void; onSection: (section: string) => void; onAddPage: () => void; onAddSection: () => void; onTheme: (id: string) => void; onPreview: () => void; onPublish: () => void }) {
  const selectedPage = project.pages.find((page) => page.id === selectedPageId) ?? project.pages[0]
  return (
    <div className="editor-layout">
      <header className="editor-topbar">
        <button type="button" className="editor-project-back" onClick={onBack}><span className="mini-brand"><BrandMark /></span><Icon name="chevron-right" size={13} /><strong>{project.title}</strong><span className="editor-status"><i />{project.status}</span></button>
        <div className="editor-tabs"><button type="button" className={editorTab === 'structure' ? 'active' : ''} onClick={() => onTab('structure')}><Icon name="layers" size={15} />Structure</button><button type="button" className={editorTab === 'design' ? 'active' : ''} onClick={() => onTab('design')}><Icon name="palette" size={15} />Design</button><button type="button" className={editorTab === 'build' ? 'active' : ''} onClick={() => onTab('build')}><Icon name="layout" size={15} />Builder</button></div>
        <div className="editor-actions"><span className="saved-state"><span className="saved-dot" />Enregistré</span><IconButton name="undo" label="Annuler" size={16} /><IconButton name="redo" label="Rétablir" size={16} /><span className="topbar-divider" /><button type="button" className="editor-preview-button" onClick={onPreview}><Icon name="play" size={14} />Aperçu</button><button type="button" className="publish-button" onClick={onPublish}>Publier <Icon name="arrow-right" size={15} /></button><div className="editor-avatar">ML</div></div>
      </header>
      {editorTab === 'structure' && <StructureView project={project} selectedPageId={selectedPageId} onPage={onPage} onAddPage={onAddPage} onTab={onTab} />}
      {editorTab === 'design' && <DesignView project={project} theme={theme} onTheme={onTheme} onTab={onTab} />}
      {editorTab === 'build' && <BuilderView project={project} theme={theme} selectedPage={selectedPage} selectedSection={selectedSection} onPage={onPage} onSection={onSection} onAddPage={onAddPage} onAddSection={onAddSection} onPreview={onPreview} />}
    </div>
  )
}
...
function BuilderView({ project, theme, selectedPage, selectedSection, onPage, onSection, onAddPage, onAddSection, onPreview }: { project: Project; theme: Theme; selectedPage: Page; selectedSection: string; onPage: (id: string) => void; onSection: (section: string) => void; onAddPage: () => void; onAddSection: () => void; onPreview: () => void }) {
  return <div className="builder-body"><EditorSidebar project={project} selectedPageId={selectedPage.id} onPage={onPage} onAddPage={onAddPage} mode="pages" /><main className="builder-main"><div className="builder-toolbar"><div><div className="mini-breadcrumb"><span>{project.title}</span><Icon name="chevron-right" size={12} /><span>{selectedPage.name}</span></div><h2>{selectedPage.name}</h2></div><div className="builder-toolbar-actions"><div className="device-switcher"><IconButton name="monitor" label="Desktop" active size={15} /><IconButton name="tablet" label="Tablette" size={15} /><IconButton name="phone" label="Mobile" size={15} /></div><button type="button" className="small-outline-button" onClick={onPreview}><Icon name="play" size={13} />Aperçu</button></div></div><div className="builder-canvas-wrap"><div className="canvas-ruler horizontal"><span>0</span><span>320</span><span>640</span><span>960</span><span>1280</span></div><div className="site-canvas" style={{ background: theme.background, color: theme.ink }}><div className="site-canvas-nav"><span className="site-logo-mark" style={{ color: theme.accent }}>✦</span><strong>{project.title}</strong><div><span>Programme</span><span>Infos</span><span>RSVP</span><button type="button" style={{ background: theme.accent, color: theme.background }}>Menu</button></div></div><div className="canvas-section selected-section" onClick={() => onSection('Hero événement')}><span className="section-tag">Hero événement <Icon name="edit" size={12} /></span><div className="canvas-hero-copy"><span className="canvas-eyebrow" style={{ color: theme.accent }}>{project.subtitle.toUpperCase()}</span><h1 style={{ fontFamily: theme.displayFont }}>{project.title.split(' & ')[0]} <em>&</em><br />{project.title.split(' & ')[1] ?? 'vous'}</h1><p>{project.date} <span>·</span> {project.location}</p><button type="button" style={{ background: theme.accent, color: theme.background }}>Découvrir l’événement <Icon name="arrow-right" size={14} /></button></div><div className="canvas-hero-art" style={{ background: theme.accentSoft }}><div className="hero-art-frame" style={{ borderColor: theme.accent }}><span>THE<br />MOMENT</span></div></div></div><div className="canvas-section canvas-info-section" onClick={() => onSection('Date & compte à rebours')}><span className="section-tag">Date & compte à rebours <Icon name="edit" size={12} /></span><div><span className="canvas-eyebrow" style={{ color: theme.accent }}>À RETENIR</span><h2 style={{ fontFamily: theme.displayFont }}>Un moment à partager.</h2></div><div className="date-boxes"><div><strong>12</strong><span>JOURS</span></div><div><strong>09</strong><span>MOIS</span></div><div><strong>26</strong><span>ANNÉE</span></div></div></div><button type="button" className="canvas-add-section" onClick={(event) => { event.stopPropagation(); onAddSection() }}><Icon name="plus" size={15} />Ajouter une section</button><div className="canvas-section canvas-story-section" onClick={() => onSection('Notre histoire')}><span className="section-tag">Notre histoire <Icon name="edit" size={12} /></span><div className="story-image" style={{ background: `linear-gradient(145deg, ${theme.accentSoft}, ${theme.accent}55)` }}><span>IMAGE / 01</span></div><div className="story-copy"><span className="canvas-eyebrow" style={{ color: theme.accent }}>LE CONTEXTE</span><h2 style={{ fontFamily: theme.displayFont }}>Tout commence<br />par une histoire.</h2><p>Ajoutez votre texte, vos images et le ton qui ressemble à votre événement.</p></div></div></div></div></main><aside className="builder-inspector"><div className="inspector-tabs"><button type="button" className="active">Style</button><button type="button">Contenu</button><button type="button">Données</button></div><div className="inspector-heading"><div className="inspector-selected-icon"><Icon name="layers" size={16} /></div><div><span>Section sélectionnée</span><strong>{selectedSection}</strong></div><IconButton name="more" label="Plus d'actions" size={16} /></div><InspectorGroup title="Disposition" icon="layout"><div className="segmented-control"><button type="button" className="active">Libre</button><button type="button">Grille</button><button type="button">Pleine largeur</button></div><div className="inspector-row"><span>Alignement</span><div className="align-buttons"><button type="button">←</button><button type="button" className="active">↔</button><button type="button">→</button></div></div><div className="range-row"><span>Espacement vertical</span><strong>96 px</strong></div><input type="range" min="20" max="160" defaultValue="96" /></InspectorGroup><InspectorGroup title="Apparence" icon="palette"><div className="inspector-row"><span>Fond de section</span><span className="color-value"><i style={{ background: theme.background }} />{theme.background.toUpperCase()}</span></div><div className="inspector-row"><span>Couleur d’accent</span><span className="color-value"><i style={{ background: theme.accent }} />{theme.accent.toUpperCase()}</span></div><div className="inspector-row"><span>Rayon des cartes</span><strong>24 px</strong></div></InspectorGroup><InspectorGroup title="Données connectées" icon="link"><div className="data-binding"><span className="binding-dot" /><div><small>Nom de l’événement</small><strong>project.title</strong></div><Icon name="check" size={14} /></div><div className="data-binding"><span className="binding-dot" /><div><small>Date principale</small><strong>event.date</strong></div><Icon name="check" size={14} /></div><button type="button" className="connect-data-button"><Icon name="link" size={14} />Connecter une donnée</button></InspectorGroup><div className="inspector-tip"><Icon name="sparkles" size={15} /><span>Les changements de style se propagent à toutes les sections qui utilisent ce composant.</span></div></aside></div>
}
